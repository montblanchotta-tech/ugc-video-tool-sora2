...
class VideoProcessor:
    def __init__(self):
        ...
        self.fishaudio = FishAudioService(api_key=Config.FISHAUDIO_API_KEY)
        self.hedra = HedraService(api_key=Config.HEDRA_API_KEY, base_url=Config.HEDRA_BASE_URL)
    ...
    async def generate_ugc_video(self, 
        ...
        try:
            ...
            # ステップ1: Gemini APIでモデル画像を生成
            logger.info(f"Step 1: Generating model image with Gemini API for video {video_id}")
            model_image_path = await self.gemini.generate_model_with_product(
                product_image_path=product_image_path,
                model_style=model_style,
                prompt="A professional model using the product naturally"
            )
            
            if not model_image_path:
                logger.error(f"Failed to generate model image for video {video_id}")
                return {"success": False, "error": "Failed to generate model image"}
            ...
            import shutil
            try:
                shutil.copy2(model_image_path, final_model_image_path)
                model_copied = True
                logger.info(f"Model image copied from {model_image_path} to {final_model_image_path}")
            except Exception as e:
                logger.error(f"Failed to copy model image: {str(e)}")
                model_copied = False
            # 音声ファイルをコピー
